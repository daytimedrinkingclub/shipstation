...
//Paypal webhook validation function
async function validatePayPalWebhook(headers, body) {
  const request = new paypal.webhooks.WebhookEvent.verifySignatureRequest();
  request.headers = headers;
  request.body = JSON.stringify(body);
  request.webhookId = process.env.PAYPAL_WEBHOOK_ID; // PayPal Webhook ID

  const response = await paypal.webhooks.WebhookEvent.verifySignature(request);
...
app.post("/paypal-webhook", async (req, res, next) => {
  try {
    // validate the webhook signature using Paypal's SDK
    ...
    // Handles the completed payment event
    if (webhookEvent.event_type === "PAYMENT.SALE.COMPLETED") {
      const email = webhookEvent.resource?.payer?.payer_info?.email;
      ...
      const webhookPayload = {
        content: "New PayPal payment received!",
        embeds: [
          {
            title: "Payment Details",
            fields: [
...
server.listen(PORT, () => {
  console.log(`Server running at port ${PORT}`);   // re-re-phrase 
});
