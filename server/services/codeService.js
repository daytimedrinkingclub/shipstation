...
async function codeAssitant({ query, filePath, client }) {
  ...
  try {
    const msg = await client.sendMessage({
      system: `Write code as per the guidelines provided, use web-components architecture with the guidelines provided by user. Never use react.
       We are limited by web-components, tailwind and fontawsome only so never use any 3rd party library other than the ones mentioned previously.
